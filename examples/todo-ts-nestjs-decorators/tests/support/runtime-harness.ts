...
const sourceProjectDir = resolve(repoRoot, "examples/todo-ts-nestjs-decorators");
const chimpbaseBunPackageDir = resolve(repoRoot);
...
export async function createProjectFixture(label: string): Promise<ProjectFixture> {
  const projectDir = await mkdtemp(join(tmpdir(), `chimpbase-todo-ts-nestjs-decorators-${label}-`));
  ...
  try {
    ...
    for (const dependency of bunRuntimeDependencies) {
      await cp(
        resolve(chimpbaseBunPackageDir, "node_modules", dependency),
        resolve(projectDir, "node_modules", dependency),
        { recursive: true },
      );
    }
