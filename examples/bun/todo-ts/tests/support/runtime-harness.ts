...
const sourceProjectDir = resolve(repoRoot, "examples/bun/todo-ts");
...
export async function createProjectFixture(label: string): Promise<ProjectFixture> {
  const projectDir = await mkdtemp(join(tmpdir(), `chimpbase-todo-ts-${label}-`));
  ...
  try {
    ...
    await rewriteExportsToSource(resolve(projectDir, "node_modules/@chimpbase/auth"));
    ...
    await rewriteExportsToSource(resolve(projectDir, "node_modules/@chimpbase/webhooks"));
