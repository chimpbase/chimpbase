...
describe("chimpbase-bun runtime", () => {
  ...
  test("registers actions, subscriptions and queues with decorators", async () => {
    const projectDir = await createInlineFixture("decorators", {
      "index.ts": [
        'import { Action, Queue, Subscription, registerFrom } from "@chimpbase/runtime";',
        "",
...
async function createFixture(label: string): Promise<string> {
  ...
  await writeFile(
    ...
    [
      ...
      'import { action, queue, register, subscription } from "@chimpbase/runtime";',
